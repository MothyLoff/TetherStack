...
/// Single source of the pull physics, not exposed in the public API.
///
/// The falloff law is inverse-square: the lead plate follows the finger,
/// neighbors attenuate by `|i - lead|`.
enum TetherPhysics {
    ...
    /// no threshold.
    ///
    /// `f(x) = sign(x) * d * atan(|x| / d)`. Slope `f'(x) = 1/(1 + (x/d)^2)`:
    /// exactly 1 at zero (finger and plate move 1:1), stays near 1 while `x` is
    /// much smaller than `d`, then eases off - resistance `1 - f'` grows like
    /// `(x/d)^2`. Travel asymptote ≈ `d * pi/2`.
